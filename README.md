...
alertmanager:
  enabled: true
  spec:
    # HA: 3 реплики. При replicaCount > 1 оператор VictoriaMetrics автоматически
    # добавляет --cluster.listen-address и --cluster.peer=<sts>-N.<svc>:9094
    # (Gossip-протокол), благодаря которому только одна из реплик отправляет
    # уведомление, остальные его дропают. Impulse (webhook-ресивер) получит одно
    # уведомление на инцидент, дублей не будет.
    replicaCount: 3
    port: "9093"
    ...
    externalURL: ""
    routePrefix: /
    # PVC на реплику (StatefulSet). Gossip синхронизирует silences и nflog между
    # репликами, но локальный storage ускоряет восстановление после рестарта и
    # хранит silence history. network-ssd — стандартный storageClass Yandex Managed K8s.
    storage:
      volumeClaimTemplate:
        spec:
          ...
          resources:
            requests:
              storage: 2Gi
    # Распределяем 3 пода по разным нодам (кластер из 3 нод) — иначе scheduler
    # может поставить их на одну ноду, что нивелирует HA при падении ноды.
    topologySpreadConstraints:
      - maxSkew: 1
        ...
        labelSelector:
          matchLabels:
            app.kubernetes.io/name: vmalertmanager-vmks
  config:
    route:
      receiver: "impulse"
      group_interval: 5m
